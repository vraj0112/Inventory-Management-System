...
<?php 
    ...
    //echo $invoiceid;
    $query = "SELECT InvoiceNo, CustomerName, InvoiceDate, Discount, TransportationCost FROM invoicemst JOIN tblcustomermst WHERE invoicemst.CustomerId = tblcustomermst.CustomerId and InvoiceId = {$invoiceid}";
    ...
    if(!$result){
        ...
        $transport = $row['TransportationCost'];
...
<body>
    <input type="text" name="" id="invoiceid" value="<?php echo $invoiceid; ?>" hidden>
            ...
            <div class="col-md-12">
                <div class="card card-primary">
                    <div class="card-header" style="background-color: #2B60DE">
                        <center>
                            <h3 class="card-title" style="color: white">Invoice No :
                                <?php echo $invoiceno ?>
                            </h3>
                        </center>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 row">
                                ...
                                <div class="col-md-4">
                                    <input type="text" class="form-control" value="<?php echo  $invoicedate ?>" disabled>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(function () {
                    ...
                    success: function (Data) {
                        ...
                        if (Data[0].FLAG == "OKK") {
                            ...
                            for (var i = 1; i < n; i++) {
                                ...
                                total = total + (parseInt(billingqty)*parseFloat(sellingprice)+totalgst);
                                console.log(total);
                                $("#purchasedtable tbody:last-child").append(
                                    ...
                                    '<td>' + date + '</td>' +
                                    '<td>' + batchno + '</td>' +
                                   // '<td  id="gstvalue">' + GST + '</td>' +
                                    '<td>' + '<span id="gstvalue">'+GST+'</span>' + '</td>' +
                            ...
                            var sub = parseFloat($("#oldTotalcost").val()) - parseFloat($("#oldDiscount").val()) + parseFloat($("#oldTransportcost").val());
            ...
            $("#savebtn").on('click', function(){
                ...
                var invoicepack = new Array();
                var invoiceid = $("#invoiceid").val();
                var total=$("#totalcost").val();
                
              //  var extracostdesc = $("#extraCostDescription").val();
                var invoicemetadata = {invoiceid: invoiceid, discount: discount, transportationcost: transportationcost, total:total};
                console.log(invoicemetadata);
                invoicepack.push(invoicemetadata);
                ...
                for (let i = 0; i < n; i++) {
                    ...
                    //ar oldotherqty   = $("#oldotherqty_"+tempstockid).html();
                   // var gst $("#gstvalue")
                    ...
                    invoicepack.push(obj);

                }

                $.ajax({
                    type: "POST",
                    url: './EditChallanAjax/editInvoiceSaveChanges.php',
                    data: {invoicedata: JSON.stringify(invoicepack)},
                    success: function(Data){
                        if(Data == "1"){
        ...
        function calsubtotal() {
            ...
            var subtotal = total - discount + Number.parseFloat(transport);
        ...
        function updateTable() {
            ...
            for (let i = 0; i < n; i++) {
                ...
                totalgst = billingQty*sellingPrice*(gst/100);
                totalprice = (totalprice + ((parseInt(billingQty)) * sellingPrice + totalgst));
            }
